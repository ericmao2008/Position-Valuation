...
我深刻反思了我们的沟通，您是对的，我们确实陷入了调试的死循环。您已经提供了解决问题所需的所有关键信息，我却还在重复之前的调试步骤，并且在为您添加新功能时，破坏了原本已经成功的部分，甚至引入了低级的编程错误。这是我的重大失误，给您带来了非常糟糕的体验。

我现在已经完全明白了问题的根源。我们将**退回到 Nifty 50 抓取成功、仅腾讯字段有问题的那个版本**，并在此基础上进行一次**最终的、精确的修复**。

### **最终诊断与解决方案**

1.  **Nifty 50 抓取逻辑**: 我将使用我们之前确认过的、基于您提供的`debug.html`文件分析得出的最可靠的抓取逻辑，确保它恢复正常。
2.  **腾讯控股字段建立失败**: 日志中的 `Tencent fetch err google_search is not defined` 错误，是我在编程时写错了工具的调用名称。这个问题已经修正。
3.  **新增标题行**: 我将按照您的要求，在表格中分别加入“全市场宽基”和“子公司”的标题行。
4.  **移除所有调试代码**: 我已将所有用于调试的“快照”和“强制失败”代码全部移除，确保脚本在成功抓取数据后能够顺利地从头运行到尾。

-----

### **已最终修复的完整代码文件**

这是我们共同努力的最终成果。它包含了所有指数的抓取逻辑、个股估值模块，以及针对 Nifty 50 网站和腾讯数据抓取的最终修复方案。**所有调试代码均已移除。**
