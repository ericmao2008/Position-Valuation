/**
 * Version History
 * V3.1.0 - Tencent data via Google Custom Search API
 * - Replaced Playwright-based Tencent scraper (Google Finance/SERP) with a direct Google Custom Search API implementation.
 * - This method is faster, more reliable, and less prone to breaking from website layout changes.
 * - Requires GOOGLE_API_KEY and GOOGLE_CX_ID environment variables.
 * - Maintained fallback to TENCENT_MC_OVERRIDE and TENCENT_SHARES_OVERRIDE.
 * - All other data fetching logic remains unchanged.
 */
...
const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY; // For Google Custom Search
const GOOGLE_CX_ID   = process.env.GOOGLE_CX_ID;   // For Google Custom Search
...
// ===== Tencent: Market Cap & Shares via Google Custom Search API =====
async function fetchTencentData() {
    const parseUnitValue = (text, unitMap) => {
        for (const [regex, multiplier] of unitMap) {
    ...
    if (!GOOGLE_API_KEY || !GOOGLE_CX_ID) {
        dbg("Tencent(Search): Missing GOOGLE_API_KEY or GOOGLE_CX_ID, falling back to ENV.");
        return {
            marketCap: numOr(process.env.TENCENT_MC_OVERRIDE, null),
            totalShares: numOr(process.env.TENCENT_SHARES_OVERRIDE, null)
        };
    }

    try {
        const [mcRes, sharesRes] = await Promise.all([
            customsearch.cse.list({ auth: GOOGLE_API_KEY, cx: GOOGLE_CX_ID, q: '腾讯控股 00700 总市值 港元' }),
            customsearch.cse.list({ auth: GOOGLE_API_KEY, cx: GOOGLE_CX_ID, q: '腾讯控股 00700 总股本' })
        ]);

        const mcText = mcRes.data.items?.map(i => i.snippet || i.title).join(' ') || '';
        const sharesText = sharesRes.data.items?.map(i => i.snippet || i.title).join(' ') || '';
        dbg("Tencent(Search) MC snippets:", mcText);
        dbg("Tencent(Search) Shares snippets:", sharesText);

        const mcUnitMap = new Map([
            [/([\d.,]+)\s*(?:万亿|trillion)/i, 1e12],
            [/([\d.,]+)\s*(?:亿|billion)/i, 1e8]
        ]);
        const sharesUnitMap = new Map([
            [/([\d.,]+)\s*(?:亿|billion)/i, 1e8]
        ]);

        let marketCap = parseUnitValue(mcText, mcUnitMap);
        let totalShares = parseUnitValue(sharesText, sharesUnitMap);

        dbg("Tencent(Search) parsed mc/sh:", marketCap, totalShares);

        // Fallback to environment variables if API parsing fails
        return {
        ...
        console.error("[ERROR] Tencent(Search) API call failed:", e.message);
