/**
 * Version History
 * V3.1.0 - Subsidiary mail line + Moutai price via Google Finance (no sheet formula)
 * - Mail now includes: "Tencent 6.01万亿 🟡 持有" (market cap in trillion HKD + judgment).
 * - Added fetchMoutaiPrice(): scrape https://www.google.com/finance/quote/SHA:600519 (via r.jina.ai).
 * - Added simple price block for 贵州茅台 under "子公司".
 * - Kept Tencent via Google-only (Finance → SERP → ENV) and partial-write behavior.
 */
...
// ===== Nikkei：PE & PB (DOM-only with Playwright) =====
...
// ===== Nifty 50: PE & PB (DOM-only with Playwright) =====
...
// ===== Tencent: Market Cap & Shares via Google (r.jina.ai text) =====
async function fetchTencentData() {
  const toNum = v => (v!=null && Number.isFinite(Number(v))? Number(v): null);
  const unitMul = u => (u==="T"?1e12: u==="B"?1e9: u==="M"?1e6: u==="K"?1e3: 1);
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (m1) {
        ...
        mc = Number.isFinite(n) ? n * unitMul((m1[3]||"").toUpperCase()) : null;
      }
      if (!mc) {
        const m2 = txt.match(/([\d.]+)\s*(trillion|billion|million)\s*(HKD|HK\$)/i);
        if (m2) {
          const n = parseFloat(m2[1]);
          const mul = m2[2].toLowerCase()==="trillion"?1e12: m2[2].toLowerCase()==="billion"?1e9: 1e6;
          mc = Number.isFinite(n) ? n * mul : null;
        }
      }
      ...
      if (sh) {
        ...
        totalShares = Number.isFinite(n) ? n * unitMul((sh[2]||"").toUpperCase()) : null;
      }
    }
  } catch(e) { dbg("Tencent(GF r.jina.ai) err", e.message); }

  // 2) Google SERP 文本兜底（市值）
  if (!marketCap) {
    try {
      ...
      if (r.ok) {
        ...
        if (!mc) {
          ...
          if (m3) {
            ...
            mc = Number.isFinite(n) ? n*unitMul((m3[2]||"").toUpperCase()) : null;
          }
        }
        marketCap = mc || marketCap;
      }
...
async function fetchMoutaiPrice() {
  try {
    ...
    if (r.ok) {
      ...
      // 备选：CNY 1481.61
      m = txt.match(/CNY\s*([\d,]+(?:\.\d+)?)/i);
      if (m) return Number(m[1].replace(/,/g,""));
    }
  } catch (e) { dbg("Moutai(GF r.jina.ai) err", e.message); }
  return null;
}
...
async function writePriceBlock(startRow, label, price, ccyLabel = "单位：元") {
  const { sheetTitle, sheetId } = await ensureToday();
  const rows = [
    ["个股", label, "真实", "个股价格分块", "—"],
    ["价格", price ?? "", "抓取", ccyLabel, "—"],
  ];
  const end = startRow + rows.length - 1;
  await write(`'${sheetTitle}'!A${startRow}:E${end}`, rows);
  const requests = [];
...
(async()=>{
  ...
  const moutai_promise  = fetchMoutaiPrice();
  ...
  if (pe_nk && pb_nk && pe_nk.v && pb_nk.v) { roe_nk.v = pb_nk.v / pe_nk.v; }
  ...
  // 10) 贵州茅台（只写价格块，价格来源：Google Finance）
  const moutaiPrice = await moutai_promise; // CNY number or null
  const res_moutai = await writePriceBlock(row, "贵州茅台", moutaiPrice, "单位：人民币（¥）");
  row = res_moutai.nextRow;
  ...
  console.log("[DONE]", todayStr(), {
    ...
    tencent_mc: res_tencent.marketCap, moutai_price: moutaiPrice
  });
  
  const roeFmt = (r) => r != null ? ` (ROE: ${(r * 100).toFixed(2)}%)` : '';
  const toWanYi = (n) => (n==null ? "-" : (n/1e12).toFixed(2)+"万亿");
  ...
  // 子公司摘要（新增）
  const tencentLine = `Tencent ${toWanYi(res_tencent.marketCap)} ${res_tencent.judgment ?? "-"}`;
  const moutaiLine  = `贵州茅台 ¥${moutaiPrice!=null ? moutaiPrice.toFixed(2) : "-"}`;

  lines.push(tencentLine);
  lines.push(moutaiLine);
