/**
 * Version History
 * V3.0.1 - Tencent via r.jina.ai (Google-only, no Playwright)
 * - Tencent market cap & shares: fetch Google Finance page text via r.jina.ai and parse.
 * - Fallback to Google SERP text via r.jina.ai.
 * - ENV overrides remain (TENCENT_MC_OVERRIDE / TENCENT_SHARES_OVERRIDE).
 * - Partial-write block for Tencent kept ("需手动补充" when incomplete).
 */
...
async function fetchTencentData() {
  ...
  // 1) Google Finance 页面文本（通过 r.jina.ai 抓取已渲染文本）
  try {
    // 注意：用 http 目标 + r.jina.ai 前缀，获取纯文本渲染结果，免 JS/Cookie
    ...
    if (r.ok) {
      ...
      // 解析 Market cap（只接受 HKD/HK$）
      let mc = null;
      // 形态1：Market cap HK$ 5.65T / HKD 5.65T
      ...
      // 形态2：5.65 trillion HKD / 950.3 billion HKD
      if (!mc) {
      ...
      // 解析 Shares outstanding
      // 形态：Shares outstanding 9.08B / 9,080,000,000
