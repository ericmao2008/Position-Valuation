...
import { JSDOM } from "jsdom"; // 需要安装：npm install jsdom
...
    const { sheetTitle, sheetId } = await ensureToday();
    const { label, ticker, totalShares, fairPE, currentProfit, growthRate, category, price, currency } = config;

    // If price is null, it means we use the GOOGLEFINANCE formula.
    // Otherwise, we use the scraped price.
    const isFormulaPrice = price === null || !Number.isFinite(price);
...
        ["价格", isFormulaPrice ? `=GOOGLEFINANCE("${ticker}", "price")` : price, isFormulaPrice ? "Formula" : "Scraped", `实时价格 (${currency})`, "Google Finance"],
...
  // ... (Other index blocks remain the same) ...
...
  // 9) 腾讯控股 (Formula-based)
  const tencentConfig = {
    label: "腾讯控股",
    ticker: "HKG:0700",
    currency: "HKD",
    price: null, // Let the sheet use the formula
...
  
  // 10) 贵州茅台 (Scraped Data)
  const moutaiPrice = await moutaiPricePromise;
  const moutaiConfig = {
    label: "贵州茅台",
    ticker: "SHA:600519",
    currency: "CNY",
    price: moutaiPrice, // Use the scraped price
...
  row = (await writeStockBlock(row, moutaiConfig)).nextRow;
  ...
  let moutaiEmailLine = "贵州茅台: 无法获取价格, 请在表格中查看";
  if (moutaiPrice) {
      ...
      moutaiEmailLine = `贵州茅台: ${(marketCap / 1e12).toFixed(2)}万亿 CNY → ${judgment}`;
  }
