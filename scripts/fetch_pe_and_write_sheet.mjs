...
const DEBUG  = String(process.env.DEBUG_VERBOSE ?? "0") === "1"; // 注意：普通空格
...
// ===== Tencent: Market Cap & Shares (Yahoo Finance API + fallback) =====
async function fetchTencentData() {
  const toNum = (v) => (v != null && Number.isFinite(Number(v)) ? Number(v) : null);

  // 1) Primary: Yahoo Finance quoteSummary API
  try {
  ...
  try {
    ...
    if (r.ok) {
      ...
      const marketCap   = toNum(res?.marketCap);
      const totalShares = toNum(res?.sharesOutstanding);

      if (marketCap && totalShares) return { marketCap, totalShares };
    }
  } catch (e) {
    dbg("Tencent(quote) err", e.message);
  }
